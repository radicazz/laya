...
The GitHub Actions in this directory implement a four-stage pipeline that mirrors the project's development process. Each stage produces the artifacts that downstream stages consume, so a failure short-circuits the rest of the pipeline.
...
- Same trigger as tests but operates on Release artifacts and produces JSON/TXT benchmark output plus metadata.
- Aggregates all benchmark outputs into a single artifact for easier consumption.
...
- Artifact retention: analysis (30 days), build/test artifacts (90 days), benchmarks (365 days), docs preview (7 days).
