...
for (const auto& event : laya::poll_events()) {
    ...
    // Type-safe, no magic constants
}
```

**Benefits:**
- Type-safe event handling
- No switch statements or magic constants
- Compiler-checked event access
- Modern C++ idioms (ranges, variants)
- Impossible to forget to handle event types
