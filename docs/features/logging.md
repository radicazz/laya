...
```cpp
// Simple logging (application category)
...
// Category-specific logging
void log(log_category category, log_priority priority,
...
void reset_log_priorities();
```

### Color Support

```cpp
bool enable_log_colors();   // Returns true if colors supported
...
class priority_guard {
    // Automatically restores priority on destruction
};

priority_guard with_log_priority(log_category category, log_priority priority);
```
